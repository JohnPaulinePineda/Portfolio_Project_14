---
title: "Supervised Learning : Cost-Sensitive Learning for Severe Class Imbalance"
...
| [Support Vector Machine](http://www.cs.cmu.edu/~pakyan/compbio/references/Drucker_NIPS_1996.pdf) plots each observation in an N-dimensional space corresponding to the number of features in the data set and finds a hyperplane that maximally separates the different classes by a maximally large margin (which is defined as the distance between the hyperplane and the closest data points from each class). The algorithm applies kernel transformation by mapping non-linearly separable data using the similarities between the points in a high-dimensional feature space for improved discrimination.
|
| [Class Weighting](https://arxiv.org/abs/2010.05995) assigns higher misclassification penalties to training instances of the minority class. The algorithm would not tend to skew the modeling estimation process towards cases from the minority class to reduce the total misclassifications, as this set of instances are now assigned with a higher misclassification penalty.
|
...
| [Classification and Regression Trees](https://www.semanticscholar.org/paper/Classification-and-Regression-Trees-Breiman-Friedman/8017699564136f93af21575810d557dba1ee6fc6) construct binary trees for both both nominal and continuous input attributes using Gini Index as its splitting criteria. The algorithm handles missing values by surrogating tests to approximate outcomes. In the pruning phase, CART uses pre-pruning technique called Cost-Complexity pruning to remove redundant branches from the decision tree to improve the accuracy. In the first stage, a sequence of increasingly smaller trees are built on the training data. In the second stage, one of these tree is chosen as the pruned tree, based on its classification accuracy on a pruning set, adopting a cross-validated method in its pruning technique.
|
| [Cost-Sensitive Learning](https://link.springer.com/referencework/10.1007/978-0-387-30164-8) assigns different costs to the types of misclassification errors that can be made, then applies specialized methods to take those costs into account. Given a penalty associated with an incorrect prediction which is referred to as the cost, the algorithm aims to minimize the cost of a model on the training data set, where it is assumed that different types of prediction errors have a different and known associated cost. The varying misclassification costs are best understood using the idea of a cost matrix.
|
| **[A]** The classification and regression trees model from the  <mark style="background-color: #CCECFF">**rpart**</mark> package was implemented through the <mark style="background-color: #CCECFF">**caret**</mark> package. This version of the model applied non-uniform case weights between the classes of the response variable (<span style="color: #FF0000">Class=M</span>:1, <span style="color: #FF0000">Class=R</span>:4). 
|
| **[B]** The model contains 1 hyperparameter:
...
|
| **[A]** Cross-validation and test results were observed to be generally unstable potentially driven by the low case numbers in the analysis.
|
| **[B]** The model which demonstrated the most consistent improvement in specificity metric after cost-sensitive training is as follows:
|      **[B.1]** **SVM_R: Support Vector Machine - Radial Basis Function Kernel**
|             **[B.1.1]** Test Specificity = 0.14286, Cross-Validation Specificity = 0.60000
|      **[B.2]** **CW_SVM_R: Class-Weighted Support Vector Machine - Radial Basis Function Kernel**
|             **[B.2.1]** Test Specificity = 0.28571, Cross-Validation Specificity = 0.65000
|
|
...
|
| ![](D:/Github_Codes/ProjectPortfolio/Portfolio_Project_14/images/Project14_Summary.png)
|
# **3. References**
|
...
| **[Book]** [Encyclopedia of Machine Learning](https://link.springer.com/referencework/10.1007/978-0-387-30164-8) by Claude Sammut and Geoffrey Webb
...
| **[R Package]** [C50](https://cran.r-project.org/web/packages/C50/C50.pdf) by Mak Kuhn
...
| **[Article]** [Cost-Sensitive SVM for Imbalanced Classification](https://machinelearningmastery.com/cost-sensitive-svm-for-imbalanced-classification/) by Jason Brownlee
...
| **[Article]** [Cost-Sensitive Learning and the Class Imbalance Problem](https://www.csd.uwo.ca/~xling/papers/cost_sensitive.pdf) by Charles Ling and Victor Sheng
...
| **[Publication]** [Class-Weighted Evaluation Metrics for Imbalanced Data Classification](https://arxiv.org/abs/2010.05995) by Akhilesh Gupta, Nesime Tatbul, Ryan Marcus, Shengtian Zhou, Insup Lee and Justin Gottschlich (Arxiv Cornell University)
